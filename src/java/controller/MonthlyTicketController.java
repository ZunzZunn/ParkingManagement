...
public class MonthlyTicketController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        // Lấy các tham số từ Form gõ vào
        String licensePlate = request.getParameter("licensePlate");
        String customerInfo = request.getParameter("customerInfo");
        String status = request.getParameter("status");

        MonthlyPassDAO dao = new MonthlyPassDAO();
        // Truyền tham số vào hàm vừa nâng cấp
        java.util.List<model.MonthlyPassDTO> passes = dao.getAllPasses(licensePlate, customerInfo, status);
